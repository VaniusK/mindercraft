from llm_providers.base_llm import *
from openai import OpenAI
import httpx
import time
import socket



class Custom(BaseLLM):
    def __init__(self, api_key : str, model : str, agent_functions : List[Callable], **kwargs):
        super().__init__(api_key, model, agent_functions, **kwargs)
        transport = httpx.HTTPTransport(local_address="0.0.0.0")
        custom_http_client = httpx.Client(transport=transport)
        self.client = OpenAI(api_key=self.api_key, http_client=custom_http_client, base_url=kwargs["base_url"]
)
        self.model = model
        self.agent_functions = agent_functions
        self.agent_functions_json = functions_to_json(agent_functions)
        self.functions_dict = {func.__name__: func for func in self.agent_functions}

    def send_message(self, history: List[Dict[Any, Any]]) -> str:
        print(history)
        start = time.time()
        response = self.client.chat.completions.create(
                model=self.model,
                messages=history,
                reasoning_effort="none",
                tools=self.agent_functions_json,
                tool_choice="required",
                extra_body={
                "reasoning": {
                    "enabled": False,
                    "exclude": False
                }
    }
            )
        print("Response time:", time.time() - start)
        print(f"Потрачено токенов (prompt): {response.usage.prompt_tokens}")
        print(f"Сгенерировано токенов (completion): {response.usage.completion_tokens}")
        
        if response.choices[0].message.content:
            print(f"Текст модели: {response.choices[0].message.content}")
        if response.choices[0].message.tool_calls:
            for tool_call in response.choices[0].message.tool_calls:
                function_name = tool_call.function.name
                function_args = json.loads(tool_call.function.arguments)
                if function_name in self.functions_dict:
                    function_to_call = self.functions_dict[function_name]
                    function_to_call(**function_args)
        reasoning_blocks = response.choices[0].message.model_extra.get('reasoning_details', [])

        for block in reasoning_blocks:
            if block.get('type') == 'text':
                print(block.get('content'))
        return format_response(response.choices[0].message.content or "") 

